...
spec=list(lfqprot.get_input_specification_tuplelist_idx__df__num_samples_quadratic__min_nonan(normed,10,1))
# process just first 50 proteins single-core: time dominated by one-time JIT compile
t=time.perf_counter(); lfqprot.get_list_with_sequential_processing(iter(spec[:50])); t1=time.perf_counter()-t
# process next 50 (warm): pure compute
t=time.perf_counter(); lfqprot.get_list_with_sequential_processing(iter(spec[50:100])); t2=time.perf_counter()-t
print(f"first50_cold={t1:.2f}s  next50_warm={t2:.2f}s  -> one-time JIT+overhead ~= {t1-t2:.2f}s per process")
