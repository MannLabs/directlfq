...
d = np.abs(vo - np.load(optbench.REF_PROT, allow_pickle=True)["vals"]); d[np.isnan(d)]=0
...
for p in diff_ids:
    ...
    vals = sub.to_numpy(); ino = sub.index.get_level_values(1).to_numpy()
    nv,_ = lfqprot._cut_peptide_values(vals, ino, 100)
    pw = 2**nv
    if np.nansum(pw)==old: ok["C"]+=1
    if np.nansum(np.asfortranarray(pw))==old: ok["F"]+=1
    if np.nansum(pw.T)==old: ok["T"]+=1
    if np.nansum(np.nansum(pw,axis=0))==old: ok["col"]+=1
