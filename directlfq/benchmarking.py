...
def get_tps_fps(result_df, prot2org_file, thresh=0.05, fc_thresh=0.3):
    ...
    for condpair in condpairs:
        ...
        num_true_positives = sum(
            annotated_fcfilt["TP"] & (annotated_fcfilt["fdr"] < 0.05)
        )
        num_false_positives = sum(
            annotated_fcfilt["FP"] & (annotated_fcfilt["fdr"] < 0.05)
        )
...
def compare_to_reference(
    result_df, reference_file, condpair
):  # put in condpair as tuple
    result_df = result_df[result_df["condpair"] == condpair]

    ref_df = pd.read_csv(reference_file, sep="\t")
    merged = pd.merge(
        result_df, ref_df, how="inner", on="protein", suffixes=["", "_ref"]
    )
...
def compare_normalization(ref_normalization_file, norm1_df, norm2_df):
    ...
    merged = pd.merge(
        norm1_df, norm2_df, how="inner", left_index=True, right_index=True
    )
...
def print_nonref_hits(protein_ref, protein_df, peptide_ref, peptide_df, outdir):
    prots_nonref_df = protein_df[
        ~(protein_df["protein"].isin(protein_ref["protein"].to_list()))
    ]  # the tilde inverts the boolean vector
    peps_nonref_df = peptide_df[
        ~(peptide_df["peptide"].isin(peptide_ref["peptide"].to_list()))
    ]
...
class ResultsTable:
    ...
    def _add_mean_intensity_column(self):
        ...
        self.formated_dataframe[self.mean_intensity_column] = self.formated_dataframe[
            ["median_intensity_c1", "median_intensity_c2"]
        ].mean(axis=1)

    def _add_median_intensity_columns_for_each_condition(self):
        self.formated_dataframe["median_intensity_c1"] = self.formated_dataframe[
            self._samples_c1
        ].median(axis=1)
        self.formated_dataframe["median_intensity_c2"] = self.formated_dataframe[
            self._samples_c2
        ].median(axis=1)

    def _add_log2fc_column(self):
        self.formated_dataframe[self.log2fc_column] = [
            np.log2(x[0]) - np.log2(x[1])
            for x in zip(
                self.formated_dataframe["median_intensity_c1"],
                self.formated_dataframe["median_intensity_c2"],
            )
        ]
...
class ResultsTableIq(ResultsTable):
    ...
    def _define_formatted_dataframe(self):
        self._load_directlfq_output()
        self.formated_dataframe[self._samples_c1 + self._samples_c2] = (
            2 ** self.formated_dataframe[self._samples_c1 + self._samples_c2]
        )
        self._add_median_intensity_columns_for_each_condition()
        self._add_log2fc_column()
        self._add_mean_intensity_column()
...
class ResultsTableMaxQuant(ResultsTable):
    ...
    def _load_maxquant_output(self):
        ...
        self.formated_dataframe = pd.read_csv(
            self._input_file, sep="\t", usecols=columns_to_use
        )
        self.formated_dataframe = self.formated_dataframe.rename(
            {"Protein IDs": self.protein_column}, axis=1
        )
...
class OrganismAnnotator(ABC):
    ...
    def _filter_double_mapping_organism(self, protein2organism_df):
        protein2organism_df = protein2organism_df[
            [
                ";" not in x
                for x in protein2organism_df[self._organism_column].astype("str")
            ]
        ]  # a semicolon seperates different organism entries
    ...
    def __add_organism_column_to_results_table(self, results_table):
        ...
        proteins_resultstable = results_table.formated_dataframe[
            results_table.protein_column
        ].astype("str")
        results_table.formated_dataframe[results_table.organism_column] = [
            protein2organism_dict.get(x) for x in proteins_resultstable
        ]

    def __get_protein2organism_dict(self):
        protein2organism = dict(
            zip(
                self._protein_organism_mapping_df[self._protein_column].astype("str"),
                self._protein_organism_mapping_df[self._organism_column],
            )
        )
    ...
    def __filter_non_matching_proteins(results_table):
        list_indicating_if_protein_matches = [
            x is not None
            for x in results_table.formated_dataframe[results_table.organism_column]
        ]
        results_table.formated_dataframe = results_table.formated_dataframe[
            list_indicating_if_protein_matches
        ]
...
class OrganismAnnotatorMaxQuant(OrganismAnnotator):
    ...
    def _load_reduce_mapping_dataframe(self):
        mapping_df = pd.read_csv(
            self._mapping_file,
            sep="\t",
            usecols=[self._protein_column, self._organism_column],
            encoding="latin1",
        ).drop_duplicates()
...
class OrganismAnnotatorDIANN(OrganismAnnotator):
    ...
    def _load_reduce_mapping_dataframe(self):
        ...
        mapping_df[self._organism_column] = [
            self.__get_organism_from_protein_name(x)
            for x in mapping_df[self._organism_column]
        ]
...
class ResultsTableBiological:
    ...
    def _load_results_table(self):
        ...
        self.results_df = self.results_df[
            [
                not x.startswith("REV_") and not x.startswith("CON_")
                for x in self.results_df[self._protein_id].astype("str")
            ]
        ]
...
class CVInfoDataset:
    ...
    def _reduce_samples_to_existing_columns(self, samples):
        existing_columns = [
            x for x in samples if x in self._results_table.results_df.columns
        ]
    ...
    def _cv_function(x):
        ...
        return np.nanstd(
            x,
            ddof=1,
        ) / np.nanmean(x)  ##ddof ensures that the sample mean std estimate is used
...
class CVDistributionPlotter:
    ...
    def _add_cv_histogram(self, dataset_cv_info):
        ...
        self._ax.hist(
            cvs,
            label=labelname,
            cumulative=self._histconfig.cumulative,
            histtype="stepfilled",
            density=self._histconfig.density,
            bins=self._histconfig.bins,
            linewidth=1.5,
            alpha=0.5,
            color=color,
        )
...
class SampleListScaler:
    ...
    def _define_number_of_replicates_of_samples(self):
        ...
        self.num_replicates_of_sample_list = math.floor(
            self._desired_number_of_samples / num_samples_in_df
        )
    ...
    def _append_remaining_sample_names_to_scaled_sample_list(self):
        self.scaled_sample_list += [
            f"{self._sample_list[x]}_AND_remainder"
            for x in range(self.num_remaining_samples)
        ]
...
class SampleIndexIQScaler(SampleListScaler):
    ...
    def _append_replicate_sample_names_to_scaled_sample_list(self):
        for idx in range(self._num_replicates_of_sample_list):
            ...
            self.scaled_sample_list += [
                x + offset_to_zero_idx for x in self._sample_list
            ]

    def _append_remaining_sample_names_to_scaled_sample_list(self):
        offset_to_zero_idx = self.scaled_sample_list[-1] + 1
        self.scaled_sample_list += [
            x + offset_to_zero_idx for x in range(self._num_remaining_samples)
        ]
...
class ScaledDFCreatorDirectLFQFormat:
    def __init__(self, template_df, desired_number_of_samples):
        ...
        self._samplelist_scaler = SampleListScaler(
            self._column_names, desired_number_of_samples
        )
        self.scaled_df = None
    ...
    def _create_scaled_dataframe(self):
        ...
        input_dataframe_dict = {
            x: template_dataframe_dict.get(
                self._samplelist_scaler.get_original_sample_name(x)
            )
            for x in self._samplelist_scaler.scaled_sample_list
        }
        self.scaled_df = pd.DataFrame(
            input_dataframe_dict, index=self._template_df.index
        )
...
class ScaledDFCreatorIQFormat:
    ...
    def _create_scaled_quant_df(self):
        ...
        self._quant_df = self._quant_df.set_index(
            "sample_list"
        )  # .loc[self._indexes_to_expand_quant_df]

        self.scaled_quant_df = self._get_scaled_quant_df(
            self._indexes_to_expand_quant_df
        )
    ...
    def _create_scaled_sample_list_df(self):
        self._samplelist_scaler = SampleListScaler(
            ...
            desired_number_of_samples=len(set(self.scaled_quant_df["sample_list"])),
        )
        self.scaled_sample_list_df = pd.DataFrame(
            {"sample_list": self._samplelist_scaler.scaled_sample_list}
        )
    ...
    def _create_indexes_to_expand_quant_df(self):
        ...
        self._indexes_to_expand_quant_df = [
            x % num_samples + 1 for x in indexes_of_scaled_sample_list
        ]
...
class LFQTimer:
    ...
    def _read_template_file(self):
        ...
        self._template_df = lfq_utils.index_and_log_transform_input_df(
            self._template_df
        )

    def _iterate_through_sizes(self):
        for samplenumber in self._samplenumbers_to_check:
            formatted_df = ScaledDFCreatorDirectLFQFormat(
                self._template_df, desired_number_of_samples=samplenumber
            ).scaled_df
            self.timed_lfq_runs.append(
                TimedLFQRun(formatted_df, self._name).run_from_formatted_df()
            )
...
class TimedLFQRun:
    ...
    def run_from_formatted_df(self):
        ...
        input_df_normed = lfqnorm.NormalizationManagerSamples(
            self._formatted_df, num_samples_quadratic=50
        ).complete_dataframe
        self.runtime_info._end_samplenorm = time.time()
        self.runtime_info._start_protein_norm = time.time()
        lfq_protein_estimation.estimate_protein_intensities(
            input_df_normed, min_nonan=1, num_samples_quadratic=10, num_cores=None
        )
...
class RuntimeInfo:
    ...
    def calculate_runtimes(self):
        ...
        self.runtime_protein_norm = (
            self._end_protein_norm - self._start_protein_norm
        ) / 60
