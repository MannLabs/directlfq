...
def get_css_style(
    ...
    directory=os.path.join(os.path.dirname(__file__), "style"),
):
...
class GUI(object):
    ...
    def __init__(
        ...
        self.layout = pn.Column(
            self.header.create(), sizing_mode="stretch_width", min_width=1270
        )
    ...
    def start_server(self, run_in_background=False):
        if self.automatic_close:
            ...
            bokeh_ws_handler.open = self.__open_browser_tab(self.bokeh_server_open)
...
class AlphaQuantGUI(GUI):
    ...
    def __init__(self, start_server=False, port=None):
        ...
        self.manual_path = os.path.join(
            os.path.dirname(__file__), "docs", "Empty_manual.pdf"
        )
