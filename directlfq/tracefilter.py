...
def exclude_unconnected_samples(
    distance_matrix,
):  # ensures that every sample in the matrix is connected to every other sample at arbitrary degree
...
def get_unconnected_sample_idxs(lower_matrix):
    ...
    check_connected_traces(
        full_matrix, starting_sample, connected_samples
    )  # fills the set
