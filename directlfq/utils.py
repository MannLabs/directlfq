...
if (
    "__file__" in globals()
):  # only run in the translated python file, as __file__ is not defined with ipython
    INTABLE_CONFIG = os.path.join(
        pathlib.Path(__file__).parent.absolute(), "configs", "intable_config.yaml"
...
try:
    ...
    warnings.warn(
        "Dependency 'dask' not installed. If you want to use its functionality, install directlfq with the 'dask' extra. Falling back to non-dask based processing."
    )
    HAS_DASK = False
...
def get_samples_used_from_samplemap_df(samplemap_df, cond1, cond2):
    samples_c1 = samplemap_df[[cond1 == x for x in samplemap_df["condition"]]][
        "sample"
    ]  # subset the df to the condition
...
def count_fraction_outliers_from_expected_fc(result_df, threshold, expected_log2fc):
    num_outliers = sum(
        [abs(x - expected_log2fc) > threshold for x in result_df["log2fc"]]
    )
...
def add_mq_protein_group_ids_if_applicable_and_obtain_annotated_file(
    mq_file, input_type_to_use, mq_protein_group_file, columns_to_add
):
    try:
        input_type = _get_input_type(mq_file, input_type_to_use)
        if (
            ("maxquant_evidence" in input_type or "maxquant_peptides" in input_type)
            and ("aq_reformat" not in mq_file)
            and ("directlfq" not in input_type_to_use)
            and (input_type_to_use != "directlfq")
        ):
            if mq_protein_group_file is None:
                ...
                mq_df = load_input_file_and_de_duplicate_if_evidence(
                    mq_file, input_type, columns_to_add
                )
                id_column = determine_id_column_from_input_df(mq_df)
                id2protein_df = create_id_to_protein_df(
                    mq_protein_group_file, id_column
                )
...
def load_input_file_and_de_duplicate_if_evidence(
    input_file, input_type, columns_to_add
):
    ...
    if "maxquant_evidence" in input_type:
        ...
        input_df = input_df_no_intensities.merge(
            input_df_grouped, how="right", left_index=True, right_index=True
        ).reset_index()
...
def create_id_to_protein_df(mq_protein_group_file, id_column):
    id_mapping_df = pd.read_csv(
        mq_protein_group_file, sep="\t", usecols=["Protein IDs", id_column]
    )
    ...
    id_mapping_df = id_mapping_df.explode(
        id_column
...
def determine_id_column_from_input_df(input_df):
    ...
    num_cols_starting_w_intensity = sum(
        [x.startswith("Intensity ") for x in input_file_columns]
    )
...
def add_columns_to_lfq_results_table(lfq_results_df, input_file, columns_to_add):
    ...
    lfq_results_df = lfq_results_df[
        [x is not None for x in lfq_results_df[config.PROTEIN_ID]]
    ]
    if (len(columns_to_add) == 0) and (len(standard_columns_for_input_type) == 0):
        return lfq_results_df
    input_df = pd.read_csv(input_file, sep="\t", usecols=all_columns).drop_duplicates(
        subset=protein_column_input_table
    )

    length_before = len(lfq_results_df.index)
    lfq_results_df_appended = pd.merge(
        lfq_results_df,
        input_df,
        left_on=config.PROTEIN_ID,
        right_on=protein_column_input_table,
        how="left",
    )
...
def show_diff(df1, df2):
    return df1.merge(df2, indicator=True, how="outer").loc[
...
def sort_input_df_by_protein_and_quant_id(data_df):
    return data_df.sort_values(
        by=[config.PROTEIN_ID, config.QUANT_ID], ignore_index=True
    )
...
def merge_protein_cols_and_ion_dict(input_df, config_dict):
    ...
    input_df = input_df.drop(
        columns=[x for x in protein_cols if x != config.PROTEIN_ID]
    )
...
def reformat_and_write_longtable_according_to_config(
    ...
    input_df_it = utils_fileread.read_file_with_pandas(
...
def process_with_dask(*, tmpfile_columnfilt, outfile_name, config_dict_for_type):
    ...
    allcols = extend_sample_allcolumns_for_plexdia_case(
...
def reshape_input_df(input_df, config_dict):
    ...
    input_df = adapt_input_df_columns_in_case_of_plexDIA(
        input_df=input_df, config_dict_for_type=config_dict
    )
    input_reshaped = pd.pivot_table(
        input_df,
        index=[config.PROTEIN_ID, config.QUANT_ID],
...
def adapt_input_df_columns_in_case_of_plexDIA(input_df, config_dict_for_type):
    if is_plexDIA_table(config_dict_for_type):
        input_df = extend_sampleID_column_for_plexDIA_case(
            input_df, config_dict_for_type
        )
...
def is_plexDIA_table(config_dict_for_type):
    return config_dict_for_type.get("channel_ID") == [
        "Channel.0",
        "Channel.4",
        "Channel.8",
    ]
...
def reformat_and_write_wideformat_table(peptides_tsv, outfile_name, config_dict):
    ...
    if "quant_pre_or_suffix" in config_dict.keys():
        ...
        headers = [config.PROTEIN_ID, config.QUANT_ID] + list(
            filter(
...
def check_for_processed_runs_in_results_folder(results_folder):
    ...
    for result_file in result_files:
        ...
        if (f"{res_name}.normed.tsv" in folder_files) & (
            f"{res_name}.results.ions.tsv" in folder_files
        ):
...
def import_data(
    input_file, input_type_to_use=None, samples_subset=None, filter_dict=None
):
    """
    ...
    file_is_already_formatted = ("aq_reformat" in input_file) | (
        input_type_to_use == "directlfq"
    )
    ...
    input_reshaped = adapt_table_for_alphabaseformat_backward_compatibility(
        file_is_already_formatted, input_reshaped
    )
...
def reformat_and_save_input_file(input_file, input_type_to_use=None, filter_dict=None):
    input_type, config_dict_for_type, sep = get_input_type_and_config_dict(
        input_file, input_type_to_use
    )

    if filter_dict is not None:
        config_dict_for_type["filters"] = dict(
            config_dict_for_type.get("filters", {}), **filter_dict
        )
    ...
    if format == "longtable":
        reformat_and_write_longtable_according_to_config(
            input_file, outfile_name, config_dict_for_type, sep=sep
        )
    elif format == "widetable":
        reformat_and_write_wideformat_table(
...
def adapt_table_for_alphabaseformat_backward_compatibility(
    file_is_already_formatted, input_reshaped
):
    if file_is_already_formatted:
...
def load_samplemap(samplemap_file):
    ...
    if "sep" not in locals():
        LOGGER.info(
            f"neither of the file extensions (.tsv, .csv, .txt) detected for file {samplemap_file}! Trying with tab separation. In the case that it fails, please add the appropriate extension to your file name."
        )
        sep = "\t"
...
def prepare_loaded_tables(data_df, samplemap_df):
    ...
    """
    samplemap_df = samplemap_df[
        samplemap_df["condition"] != ""
    ]  # remove rows that have no condition entry
    filtvec_not_in_data = [
        (x in data_df.columns) for x in samplemap_df["sample"]
    ]  # remove samples that are not in the dataframe
...
class LongTableReformater:
    ...
    def reformat_and_save_acquisition_data_frame(self, output_file):
        ...
        for input_df_subset in input_df_it:
            input_df_subset = self._reformatting_function(input_df_subset)
            self.__write_reformatted_df_to_file__(
                input_df_subset, output_file, write_header
            )
            write_header = False

    def __initialize_df_iterator__(self):
        return pd.read_csv(
            self._input_file, sep="\t", encoding="latin1", chunksize=1000000
        )

    @staticmethod
    def __write_reformatted_df_to_file__(reformatted_df, filepath, write_header):
        reformatted_df.to_csv(
            filepath, header=write_header, mode="a", sep="\t", index=None
        )
