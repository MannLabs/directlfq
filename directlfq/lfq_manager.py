__all__ = [
    "run_lfq",
    "prepare_input_filename",
    "get_outfile_basename",
    "save_protein_df",
    "save_ion_df",
    "save_run_config",
]
...
def run_lfq(
    input_file,
    columns_to_add=[],
    selected_proteins_file: str = None,
    mq_protein_groups_txt=None,
    min_nonan=1,
    ...
    input_file = (
        lfqutils.add_mq_protein_group_ids_if_applicable_and_obtain_annotated_file(
            input_file, input_type_to_use, mq_protein_groups_txt, columns_to_add
        )
    )
    input_df = lfqutils.import_data(
    ...
    if not deactivate_normalization:
        ...
        input_df = lfqnorm.NormalizationManagerSamplesOnSelectedProteins(
            input_df,
            num_samples_quadratic=number_of_quadratic_samples,
            selected_proteins_file=selected_proteins_file,
        ).complete_dataframe
    ...
    try:
        protein_df = lfqutils.add_columns_to_lfq_results_table(
            protein_df, input_file, columns_to_add
        )
    except:
        LOGGER.info(
    ...
    outfile_basename = get_outfile_basename(
        input_file,
        input_type_to_use,
...
def prepare_input_filename(input_file):
    input_file = (
        rf"{input_file}".replace("\ ", " ").rstrip()
    )  # replace escaped spaces with normal spaces and remove trailing whitespace
    return input_file


def get_outfile_basename(
    input_file,
    input_type_to_use,
    selected_proteins_file,
    deactivate_normalization,
    ...
    outfile_basename += (
        ".selected_proteins" if selected_proteins_file is not None else ""
    )
...
def save_protein_df(protein_df, outfile_basename):
    protein_df.to_csv(
        f"{outfile_basename}.protein_intensities.tsv", sep="\t", index=None
    )
...
def save_run_config(outfile_basename, kwargs):
    ...
    try:
        df_configs = pd.DataFrame.from_dict(
            simple_kwargs, orient="index", columns=["value"]
        )
