...
class AlphaPeptColorMap:
    def __init__(self):
        ...
        self.colormap_linear = matplotlib.colors.LinearSegmentedColormap.from_list(
            "alphapept", self.colorlist
        )
        self.colormap_discrete = matplotlib.colors.LinearSegmentedColormap.from_list(
            "alphapept", self.colorlist, N=5
        )
...
class CmapRegistrator:
    ...
    def _define_existing_cmaps(self):
        self._existing_matplotlib_cmaps = [
            name for name in plt.colormaps() if not name.endswith("_r")
        ]
...
class IonTraceCompararisonPlotter:
    ...
    def _define_protein_df_before_norm(self):
        self._protein_df_before_norm = pd.DataFrame(
            self._proteome_df.loc[self._selected_protein]
        )
        self._protein_df_before_norm = self._protein_df_before_norm.dropna(
            axis="columns", how="all"
        )

    def _define_protein_df_after_norm(self):
        self._protein_df_after_norm = lfq_norm.NormalizationManagerProtein(
            self._protein_df_before_norm.copy(), num_samples_quadratic=10
        ).complete_dataframe
        self._protein_df_after_norm = self._protein_df_after_norm.dropna(
            axis="columns", how="all"
        )
...
class IonTraceCompararisonPlotterNoDirectLFQTrace(IonTraceCompararisonPlotter):
    ...
    def _plot_after_norm(self):
        visualizer = IonTraceVisualizer(
            self._protein_df_before_norm, ax=self.axis_normed
        )
...
class IonTraceVisualizer:
    ...
    def _plot_ion_traces(self):
        ...
        for idx in range(plot_values.shape[0]):
            ...
            self._ax.plot(
                x_values[nan_mask], y_values[nan_mask], color="grey", alpha=0.5
            )
            self._ax.scatter(
                x_values[nan_mask], y_values[nan_mask], color="grey", marker="o", s=11
            )
    ...
    def add_median_trace(self, list_of_median_values):
        sns.lineplot(
            x=range(len(list_of_median_values)),
            y=list_of_median_values,
            ax=self._ax,
            color="black",
            linewidth=3,
        )
...
class MultiOrganismMultiMethodBoxPlot:
    def __init__(
        self, method_ratio_results_table, ax, organisms_to_plot, fcs_to_expect
    ):
        ...
        self._colorlist_hex = [
            ...
            "#325e7a",
            "#ffd479",
        ] + AlphaPeptColorMap().colorlist_hex
    ...
    def plot_boxplot(self):
        color_palette = sns.color_palette(
            self._colorlist_hex, n_colors=len(self._fcs_to_expect)
        )
        sns.violinplot(
            data=self._method_ratio_results_table,
            x="method",
            y="log2fc",
            hue="organism",
            palette=color_palette,
            hue_order=self._organisms_to_plot,
            ax=self.ax,
        )
...
def plot_withincond_fcs(normed_intensity_df, cut_extremes=True):
    ...
    for spair in samplecombs:  # compare all pairs of samples
        ...
        s2 = spair[1]
        diff_fcs = (
            normed_intensity_df[s1].to_numpy() - normed_intensity_df[s2].to_numpy()
        )  # calculate fold changes by subtracting log2 intensities of both samples

        if cut_extremes:
            cutoff = max(
                abs(np.nanquantile(diff_fcs, 0.025)),
                abs(np.nanquantile(diff_fcs, 0.975)),
            )  # determine 2.5% - 97.5% interval, i.e. remove extremes
            range = (-cutoff, cutoff)
        else:
            range = None
        plt.hist(
            diff_fcs, 80, density=True, histtype="step", range=range
        )  # set the cutoffs to focus the visualization
...
class MultiOrganismIntensityFCPlotter:
    def __init__(
        self,
        ax,
        resultstable_w_ratios,
        organisms_to_plot=None,
        fcs_to_expect=None,
        title="",
    ):
        self.ax = ax
    ...
    def _get_organisms_to_plot(self, organisms_to_plot):
        if organisms_to_plot is not None:
            ...
            return sorted(
                list(
                    set(
                        self._resultstable_w_ratios.formated_dataframe[
                            self._organism_column
                        ].astype("str")
                    )
                )
            )
    ...
    def _scatter_per_organism(self):
        ...
        complete_table[self._mean_intensity_column] = np.log2(
            complete_table[self._mean_intensity_column]
        )
        complete_table = self._remove_omitted_organisms_from_table(complete_table)
        color_palette = sns.color_palette(
            self._color_list_hex, n_colors=len(self._organisms_to_plot)
        )
        sns.scatterplot(
            data=complete_table,
            x=self._mean_intensity_column,
            y=self._log2fc_column,
            hue=self._organism_column,
            alpha=0.15,
            ax=self.ax,
            hue_order=self._organisms_to_plot,
            palette=color_palette,
    ...
    def _remove_omitted_organisms_from_table(self, complete_table):
        row_w_permitted_organism = [
            x in self._organisms_to_plot for x in complete_table["organism"]
        ]
