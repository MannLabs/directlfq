...
class ProteinProfileGenerator:
    ...
    def _generate_protein_profile_dataframe(self):
        ...
        self.protein_profile_dataframe = pd.DataFrame(
            collected_profiles, index=[protnames_for_index, pepnames_for_index]
        )
        self.protein_profile_dataframe.index.names = ["protein", "ion"]
        self.protein_profile_dataframe = np.log2(
            self.protein_profile_dataframe.replace(0, np.nan)
        )
...
class PeptideProfile:
    def __init__(
        self,
        protein_name,
        fraction_zeros_in_profile,
        systematic_peptide_shift,
        add_noise,
        num_samples=20,
        min_intensity=1e6,
        max_intensity=1e10,
    ...
    def _get_single_peptide_profile_template(self):
        ...
        return rs.randint(
            low=self._min_intensity,
            high=self._max_intensity,
    ...
    def _scale_profile_vector(self):
        self.peptide_profile_vector = (
            self.peptide_profile_vector * self._systematic_peptide_shift
        )
    ...
    def _apply_poisson_noise_to_profilevector(self):
        ...
        self.peptide_profile_vector = rng.poisson(
            lam=self.peptide_profile_vector, size=len(self.peptide_profile_vector)
        )

    def _add_zeros_to_profilevector(self):
        num_elements_to_set_zero = int(
            self._num_samples * self._fraction_zeros_in_profile
        )
        idxs_to_set_zero = np.random.choice(
            self._num_samples, size=num_elements_to_set_zero, replace=False
        )
...
class RatioChecker:
    def __init__(
        self, formatted_df: pd.DataFrame, organism2expectedfc: dict, organism2CI95: dict
    ):
    ...
    def _check_ratio_consistency_per_organism(self):
        for organism in self._organism2expectedfc.keys():
            ...
            RatioConsistencyChecker(
                fcs=fcs, expected_fc=expected_fc, deviation_threshold=deviation_threshod
            )
...
class RatioConsistencyChecker:
    def __init__(self, fcs, expected_fc, deviation_threshold):
        ...
        self._fc_deviation_center = (
            self._calc_deviation_center()
        )  # should be 0 if no bias
    ...
    def _assert_no_bias(self):
        assert self._fc_deviation_center < self._deviation_threshold, (
            f"Deviation from center: {self._fc_deviation_center:.2f}"
        )
        assert self._fraction_consistent > 0.95, (
            f"Fraction consistent below 95%: {self._fraction_consistent:.2f}"
        )
