...
class HeaderWidget(object):
    ...
    def __init__(self, title, img_folder_path, github_url):
        self.header_title = pn.pane.Markdown(
            f"# {title}",
            sizing_mode="stretch_width",
            align="center",  # Center the title
        )
        self.biochem_logo_path = os.path.join(img_folder_path, "mpi_logo.png")
        self.mpi_logo_path = os.path.join(
            img_folder_path, "max-planck-gesellschaft.jpg"
        )
        self.github_logo_path = os.path.join(img_folder_path, "github.png")
...
class RunPipeline(BaseWidget):
    ...
    def run_pipeline(self, *args):
        ...
        mq_protein_groups_txt = (
            None
            if self.path_protein_groups_file.value_input == ""
            else self.path_protein_groups_file.value_input
        )
        additional_headers = (
            None
            if self.additional_headers.value_input == ""
            else self.additional_headers.value_input
        )
        min_nonan = self.num_nonan_vals.value
        file_of_proteins_for_normalization = (
            None
            if self.protein_subset_for_normalization_file.value_input == ""
            else self.protein_subset_for_normalization_file.value_input
        )
        num_cores = (
            None if self.num_cores_vals.value == -1 else self.num_cores_vals.value
        )
        yaml_filt_dict_path = (
            None
            if self.yaml_filt_dict_path.value == ""
            else self.yaml_filt_dict_path.value
        )
        if (
            additional_headers is not None
        ):  # the user will enter a string with semicolon separated values
...
class Tabs(object):
    ...
    def create_layout(self, *args):
        if (
            self.pipeline.path_output_folder.value is not None
