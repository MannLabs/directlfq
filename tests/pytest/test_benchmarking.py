...
def test_that_scaled_numbers_of_samples_are_as_expected(
    quant_df, samplelist_df, desired_num_samples
):
    scaled_df_creator = lfq_benchmarking.ScaledDFCreatorIQFormat(
        quant_df, samplelist_df, desired_num_samples
    )
    assert (
        len(set(scaled_df_creator.scaled_quant_df["sample_list"]))
        == desired_num_samples
    )
...
def test_that_values_are_as_expected(template_df, num_samples):
    ...
    assert np.allclose(
        template_df.loc[:, "BoxCar_02-01_2"],
        size_adjusted_df.loc[:, "BoxCar_02-01_2_AND_remainder"],
    )
